...
import { workHistoryDetail } from '@/providers/logged-in/candidate.service';
...
import { AssignmentDetail } from '@/components/app/profile/assignment-detail';
...
const AssignmentPage = () => {
    ...
    const [history, setHistory] = useState<CandidateWorkHistory | null>(null);
    ...
    useEffect(() => {
        ...
        return () => {
            track('page_exit', { page: 'Assignment Page' });
        }
    }, [id]);
    ...
    const onSegmentChange = (segment: string) => {
        ...
        if (segment == 'logs' && candidateWorkingDates.length == 0) {
            loadPage(1);
        }
    };
    ...
    const loadPage = (page: number) => {
        ...
        setLoading(true);
        listWorkingDates(page, getUrlParams()).then((res: any) => {
             
            setCandidateWorkingDates(groupBy(res.data, 'date'));
    ...
    const getUrlParams = () => {
        ...
        let url = '&candidate_id=' + history?.candidate_id + 
    ...
    //meta tags?  {history ? history.store.store_name : history.company.company_name}
        ...
        <Suspense fallback={<Loading />}>
            ...
            <div className=' bg-white'>
                <div className="max-w-4xl mx-auto px-6 shadow-[0px_10px_20px_0px_rgba(0,0,0,0.05) xs:pt-0 sm:pt-6">
                    ...
                    <Segments segments={segments} onChange={(segment) => onSegmentChange(segment)} segment={segment} />
                </div>    
            </div>
            ...
            <div className="max-w-4xl mx-auto p-6">
                ...
                {segment === 'details' && (
                    <>
                        { history ? <AssignmentDetail history={history} />: <>Loading...</> }
                    </>
                ) }
